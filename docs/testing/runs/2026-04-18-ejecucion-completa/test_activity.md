...
Feature: POST /activities/no-ludica
  ...
  Scenario: Given valid request with exercise When generating no ludica Then returns NoLudicaResponseDto with 201 Created
...
Feature: NoLudicaRequestDto validation
  ...
  Scenario: Given valid request with exercise When validating Then no violations are returned
