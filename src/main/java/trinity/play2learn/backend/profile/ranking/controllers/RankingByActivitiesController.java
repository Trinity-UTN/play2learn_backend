...
@RestController
@AllArgsConstructor
@RequestMapping("/ranking/activities")
public class RankingByActivitiesController {
    
    private final IRankingByActivitiesService rankingByActivitiesService;

    @GetMapping({"/{type}", "/{type}/{id}"})
    @SessionRequired(roles = {Role.ROLE_STUDENT})
    public ResponseEntity<BaseResponse<LeaderboardResponseDto>> getRankingByActivities(
        @PathVariable LeaderboardType type,
        @PathVariable(required = false) Long id,
        @SessionUser User user
    ) {
        return ResponseFactory.ok(
            rankingByActivitiesService.cu121rankingByActivities(LeaderboardRequestMapper.toDto(type, id), user),
