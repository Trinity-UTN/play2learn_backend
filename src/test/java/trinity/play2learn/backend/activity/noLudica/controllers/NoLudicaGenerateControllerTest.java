...
class NoLudicaGenerateControllerTest {
    ...
    class GenerateNoLudica {
        ...
        @Test
        @DisplayName("Given valid request with exercise When generating no ludica Then returns NoLudicaResponseDto with 201 Created")
        void shouldGenerateNoLudicaSuccessfully() throws Exception {
            ...
            when(noLudicaGenerateService.cu45GenerateNoLudica(
                    ...
                    ArgumentMatchers.any(User.class))).thenReturn(responseDto);
            ...
            mockMvc.perform(post(BASE_PATH)
                    ...
                    .andExpect(jsonPath("$.data.exercise").value(requestDto.getExercise()))
                    .andExpect(
                            jsonPath("$.message").value(SuccessfulMessages.createdSuccessfully("Actividad no ludica")));
