...
@WebMvcTest(SavingAccountWithdrawalController.class)
@AutoConfigureMockMvc(addFilters = false)
class SavingAccountWithdrawalControllerTest {
    ...
    @Nested
    @DisplayName("POST /investment/saving-accounts/withdrawal")
    class PostWithdrawal {

        @Test
        @DisplayName("Given valid withdrawal request When withdrawing Then returns 201 Created with updated saving account response")
        void whenValidRequest_returnsCreatedWithResponse() throws Exception {
            // Given - Request válido
            SavingAccountWithdrawalRequestDto requestDto = InvestmentTestMother
                    .savingAccountWithdrawalRequestDto(InvestmentTestMother.DEFAULT_SAVING_ACCOUNT_ID,
            ...
            Mockito.when(savingAccountWithdrawalService.cu104withdrawalSavingAccount(Mockito.any(),
                    Mockito.any(User.class))).thenReturn(responseDto);

            // When & Then - POST debe retornar 201 con respuesta
            mockMvc.perform(post("/investment/saving-accounts/withdrawal")
            ...
            Mockito.verify(savingAccountWithdrawalService).cu104withdrawalSavingAccount(Mockito.any(),
                    Mockito.any(User.class));
        }

        @Test
        @DisplayName("Given invalid withdrawal request When withdrawing Then returns 400 Bad Request")
        ...
        @Test
        @DisplayName("Given non-existing saving account When withdrawing Then returns 404 Not Found")
        ...
        @Test
        @DisplayName("Given insufficient saving account balance When withdrawing Then returns 400 Bad Request")
        void whenInsufficientBalance_returnsBadRequest() throws Exception {
            ...
            Mockito.when(savingAccountWithdrawalService.cu104withdrawalSavingAccount(Mockito.any(),
                    Mockito.any(User.class)))
                    .thenThrow(new BadRequestException("No hay suficiente balance en la caja de ahorro"));
