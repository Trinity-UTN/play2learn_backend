...
import trinity.play2learn.backend.admin.subject.services.commons.SubjectGetStudentsService;
...
class StudentListPaginatedServiceTest {
        ...
        @Mock
        private SubjectGetStudentsService subjectGetStudentsService;
        ...
        class ListPaginatedStudents {
                ...
                void whenSearchAndFiltersProvided_returnsPaginatedData() {
                        ...
                        StudentResponseDto studentDto = StudentResponseDto.builder()
                                        ...
                                        .active(true)
                                        .build();
                        PaginatedData<StudentResponseDto> expectedPaginatedData = PaginatedData
                        ...
                        try (
                                        MockedStatic<PaginatorUtils> paginatorUtilsMock = mockStatic(
                                                        PaginatorUtils.class);
                                        MockedStatic<StudentSpecs> studentSpectsMock = mockStatic(StudentSpecs.class);
                                        MockedStatic<StudentMapper> studentMapperMock = mockStatic(StudentMapper.class);
                                        MockedStatic<PaginationHelper> paginationHelperMock = mockStatic(
                                                        PaginationHelper.class)) {
                                paginatorUtilsMock.when(
                                                () -> PaginatorUtils.buildPageable(PAGE, SIZE, ORDER_BY, ORDER_TYPE))
                                                .thenReturn(pageable);
                                ...
                                studentSpectsMock.when(() -> StudentSpecs.nameOrLastNameContains(search))
                                                .thenReturn(nameSpec);
                                studentSpectsMock.when(() -> StudentSpecs.genericFilter("courseId", "20"))
                                                .thenReturn(courseSpec);
                                studentSpectsMock.when(() -> StudentSpecs.genericFilter("active", "true"))
                                                .thenReturn(activeSpec);

                                studentMapperMock.when(() -> StudentMapper.toListDto(pageResult.getContent()))
                                                .thenReturn(List.of(studentDto));
                                paginationHelperMock
                                                .when(() -> PaginationHelper.fromPage(pageResult, List.of(studentDto)))
                                                .thenReturn(expectedPaginatedData);

                                PaginatedData<StudentResponseDto> result = studentListPaginatedService
                                                .cu21ListPaginatedStudents(
                                                                PAGE, SIZE, ORDER_BY, ORDER_TYPE, search, filters,
                                                                filterValues);

                                paginatorUtilsMock.verify(
                                                () -> PaginatorUtils.buildPageable(PAGE, SIZE, ORDER_BY, ORDER_TYPE));
                                studentSpectsMock.verify(() -> StudentSpecs.nameOrLastNameContains(search));
                                studentSpectsMock.verify(() -> StudentSpecs.genericFilter("courseId", "20"));
                                studentSpectsMock.verify(() -> StudentSpecs.genericFilter("active", "true"));
                                studentMapperMock.verify(() -> StudentMapper.toListDto(pageResult.getContent()));
                                paginationHelperMock.verify(
                                                () -> PaginationHelper.fromPage(pageResult, List.of(studentDto)));
                ...
                void whenSearchBlankAndFiltersMismatch_skipsSpecifications() {
                        ...
                        try (
                                ...
                                paginatorUtilsMock.when(
                                                () -> PaginatorUtils.buildPageable(PAGE, SIZE, ORDER_BY, ORDER_TYPE))
                                                .thenReturn(pageable);
                                studentMapperMock.when(() -> StudentMapper.toListDto(emptyPage.getContent()))
                                                .thenReturn(List.of());
                                ...
                                PaginatedData<StudentResponseDto> result = studentListPaginatedService
                                                .cu21ListPaginatedStudents(
                                                                PAGE, SIZE, ORDER_BY, ORDER_TYPE, "   ",
                                                                List.of("courseId"), List.of());
